...
void Viewer::updateContentSize()
{
    ...
    if (currentPage != 0 && !currentPage->isNull()) {
        ...
        {
            QPixmap page = currentPage->scaled(content->width() * devicePixelRatio(),
...
void Viewer::scrollZigzag(scrollDirection d1,
    scrollDirection d2,
    bool forward)
{
    if (!isEdge(d1)) {
        ...
        if (d2 == UP)
            y = std::max(verticalScrollBar()->sliderPosition() - static_cast<int>((height() * 0.80)),
                verticalScrollBar()->minimum());
        else if (d2 == DOWN)
            y = std::min(verticalScrollBar()->sliderPosition() + static_cast<int>((height() * 0.80)),
                verticalScrollBar()->maximum());
        else if (d2 == LEFT)
            x = std::max(horizontalScrollBar()->sliderPosition() - static_cast<int>((width() * 0.80)),
                horizontalScrollBar()->minimum());
        else // d2 == RIGHT
            x = std::min(horizontalScrollBar()->sliderPosition() + static_cast<int>((width() * 0.80)),
...
void Viewer::keyPressEvent(QKeyEvent* event)
{
    if (render->hasLoadedComic()) {
        ...
        if (key == ShortcutsManager::getShortcutsManager().getShortcut(AUTO_SCROLL_FORWARD_ACTION_Y)) {
            ...
            nextPos = verticalScrollBar()->sliderPosition() + static_cast<int>((height() * 0.70));
            scrollDown();
        }
        ...
        else if (key == ShortcutsManager::getShortcutsManager().getShortcut(MOVE_DOWN_ACTION_Y) || key == ShortcutsManager::getShortcutsManager().getShortcut(MOVE_UP_ACTION_Y) || key == ShortcutsManager::getShortcutsManager().getShortcut(MOVE_LEFT_ACTION_Y) || key == ShortcutsManager::getShortcutsManager().getShortcut(MOVE_RIGHT_ACTION_Y)) {
        ...
        if (showGoToFlowAnimation->state() != QPropertyAnimation::Running) {
            if (Configuration::getConfiguration().getDisableShowOnMouseOver() == false) {
                if (goToFlow->isVisible()) {
                    ...
                    if (gtfPos.y() < 0 || gtfPos.x() < 0 || gtfPos.x() > goToFlow->width()) // TODO this extra check is for
